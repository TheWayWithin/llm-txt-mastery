...
async function testEmailFlowForTier(page, tierName, expectedEmailCapturedEvent = true) {
  ...
  try {
    ...
    const emailCaptureCard = page.locator('[data-testid="email-capture"], .card, [class*="card"]').first();
    ...
    // Submit the form
    const submitButton = page.locator('button[type="submit"], button:has-text("Start Analysis"), button:has-text("Continue to Payment"), button:has-text("Pay")').first();
    ...
    if (tierName === 'coffee') {
      ...
      await page.waitForTimeout(3000);
      const urlInputVisible = await page.locator('input[placeholder*="website"], input[id*="url"], input[name*="url"]').isVisible().catch(() => false);
      const analysisVisible = await page.locator('text=Analysis, text=Analyzing, [data-testid*="analysis"]').isVisible().catch(() => false);
...
async function runEmailFlowTests() {
  ...
  Object.entries(testResults).forEach(([tier, result]) => {
    ...
    const emailEvent = result.emailCapturedFired ? '✅ EMAIL_CAPTURED fired' : '❌ EMAIL_CAPTURED missing';
