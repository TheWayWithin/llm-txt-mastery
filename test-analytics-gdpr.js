  ...
  try {
    ...
    const gtmInfo = await page.evaluate(() => {
      return {
        ...
        gtmNoscript: !!document.querySelector(
    ...
    const enzuzoPresent = await page.evaluate(() => {
      return (
        typeof window.EnzuzoSDK !== 'undefined' ||
    ...
    console.log(
      `✅ GTM Events: ${gtmInfo.dataLayerLength >= 3 ? 'PASS' : 'FAIL'} (${gtmInfo.dataLayerLength} events)`
    );
    console.log(
      `✅ Consent Banner: ${bannerVisible ? 'PASS' : 'PENDING'} (Enzuzo not configured yet)`
    );
    console.log(`✅ Privacy Links: ${privacyLinks > 0 ? 'PASS' : 'PENDING'}`);
    console.log(
      `✅ Event Tracking: ${finalDataLayer.length > 3 ? 'PASS' : 'PENDING'} (Custom events need tier selection)`
    );
