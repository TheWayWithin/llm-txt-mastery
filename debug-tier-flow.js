...
async function debugTierFlow() {
  ...
  try {
    ...
    if (await emailInput.isVisible({ timeout: 5000 })) {
      ...
      // Click continue
      const continueBtn = await page.locator('button:has-text("Continue without password")').first();
    ...
    const urlInputVisible = await page.locator('input[placeholder*="website URL"]').first().isVisible({ timeout: 2000 });
    console.log(`   URL input visible: ${urlInputVisible}`);
    
    // Check for any error messages
    const errorElements = await page.locator('[class*="error"], [class*="Error"], .text-red-').all();
