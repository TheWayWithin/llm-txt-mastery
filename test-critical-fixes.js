...
async function runTests() {
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      ...
      if (errorText.includes('useRoutes') || 
  ...
  try {
    ...
    for (const testPage of testPages) {
      try {
        ...
        if (hasContent) {
          ...
          console.log(`❌ FAILURE: ${testPage.name} page (${testPage.path}) failed to load properly`);
        }
        
      } catch (error) {
        console.log(`❌ ERROR: Failed to navigate to ${testPage.name} (${testPage.path}): ${error.message}`);
      }
    }
    ...
    if (consoleErrors.length <= 2) {  // Allow minor non-critical errors
      console.log('✅ Console Errors: ACCEPTABLE');
    } else {
      console.log('❌ Console Errors: NEEDS ATTENTION');
    }
    
    const overallStatus = routerErrors.length === 0 && consoleErrors.length <= 2 ? 
