...
async function inspectEmailForm() {
  ...
  try {
    ...
    if (await starterRadio.count() > 0) {
      ...
      const starterLabel = page.locator('text=/starter/i, text=/test drive/i, text=/free/i').first();
    ...
    if (await submitButton.count() > 0) {
      ...
      const emailCapturedMessages = consoleMessages.filter(msg => 
      ...
      const stateMessages = consoleMessages.filter(msg => 
