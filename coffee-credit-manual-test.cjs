...
async function testCreditsAPI() {
  ...
  try {
    ...
    if (response.data.tier === 'coffee') {
      ...
      if ('creditsRemaining' in response.data) {
        console.log(
          `✅ FIXED: creditsRemaining field is present with value: ${response.data.creditsRemaining}`
        );
  ...
  try {
    ...
    if (response.data.tier === 'coffee') {
      ...
      console.log(
        `ℹ️ Test account is ${response.data.tier} tier (expected for non-existent users)`
      );
    }
  } catch (error) {
