...
test.describe('Coffee Tier Credit System - Production Tests', () => {
  ...
  test('1. Admin Credit Reset - Test admin endpoint functionality', async ({ request }) => {
    ...
    const response = await request.post(
      `${PRODUCTION_BACKEND}/api/auth/admin/reset-coffee-credits`,
      {
        headers: {
  ...
  test('5. Credit Display - Login and verify credits display', async ({ page }) => {
    ...
    const coffeeTierIndicators = ['text=/Coffee/', 'text=/coffee/', 'text=/COFFEE/'];
  ...
  test('6. Credit Consumption - Perform analysis and verify decrement', async ({ page }) => {
    ...
    const usageResponse = await page.request.get(
      `${PRODUCTION_BACKEND}/api/usage/${encodeURIComponent(JAMIE_EMAIL)}`
    );
    if (usageResponse.ok()) {
    ...
    try {
      ...
      const afterUsageResponse = await page.request.get(
        `${PRODUCTION_BACKEND}/api/usage/${encodeURIComponent(JAMIE_EMAIL)}`
      );
  ...
  test('8. Monthly Renewal Simulation - Test Stripe webhook', async ({ request }) => {
    ...
    const renewalResponse = await request.post(
      `${PRODUCTION_BACKEND}/api/auth/admin/reset-coffee-credits`,
      {
        headers: {
          'x-admin-key': ADMIN_KEY,
