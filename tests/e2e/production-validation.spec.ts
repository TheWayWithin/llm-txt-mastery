...
test.describe('Production Validation Tests', () => {
  test.describe('Landing Page Tests', () => {
    test('should display solopreneur messaging and branding', async ({ page }) => {
      ...
      const trustIndicators = page.locator(
        '[data-testid="trust-indicators"], .trust-indicators, .testimonials'
      );
    ...
    test('should have functional email capture form', async ({ page }) => {
      ...
      const submitButton = page
        .locator(
          'button[type="submit"], button:has-text("Get Started"), button:has-text("Continue"), button:has-text("Start")'
        )
        .first();
    ...
    test('should display tier selection with correct options', async ({ page }) => {
      ...
      if ((await emailInput.count()) > 0) {
        ...
        const continueButton = page
          .locator(
            'button[type="submit"], button:has-text("Get Started"), button:has-text("Continue")'
          )
          .first();
      ...
      const freeTier = page
        .getByText('Test Drive', { exact: false })
        .or(page.getByText('Free', { exact: false }));
      const coffeeTier = page
        .getByText('Solopreneur Special', { exact: false })
        .or(page.getByText('Coffee', { exact: false }));
  ...
  test.describe('Free Tier Flow Test', () => {
    test('should complete free tier analysis workflow', async ({ page }) => {
      ...
      const freeTierButton = page.locator(
        'button:has-text("Test Drive"), button:has-text("Free"), [data-tier="free"]'
      );
      ...
      const urlInput = page.locator(
        'input[type="url"], input[placeholder*="website" i], input[placeholder*="url" i]'
      );
      if ((await urlInput.count()) > 0) {
        ...
        const analyzeButton = page.locator(
          'button:has-text("Analyze"), button:has-text("Start Analysis"), button[type="submit"]'
        );
    ...
    test('should be mobile responsive on key pages', async ({ page, isMobile }) => {
      ...
      for (const testPage of testPages) {
        ...
        const mobileMenuButton = page.locator(
          'button[aria-label*="menu" i], .menu-toggle, .hamburger'
        );
