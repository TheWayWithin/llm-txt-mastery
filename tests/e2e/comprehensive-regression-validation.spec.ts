    ...
    try {
      ...
      if (coffeeVisible) {
        await reporter.recordResult(
          ...
          'PASS',
          'Coffee tier visible and default'
        );

        // Verify Coffee tier is default/pre-selected
        const coffeeSelected = await page
          .locator('[data-testid="tier-coffee"]')
      ...
      const continueButton = page
        .locator('button:has-text("Continue with Coffee"), button:has-text("Get Started")')
        .first();
      ...
      if (currentUrl.includes('dashboard')) {
        ...
        const breadcrumbVisible = await page
          .locator('[data-testid="progress-breadcrumb"]')
          .isVisible();
  ...
  test('2️⃣ GDPR COMPLIANCE VALIDATION', async ({ page }) => {
    ...
    try {
      ...
      const consentBanner = page.locator(
        '.consent-banner, [data-testid="consent-banner"], #consent-banner'
      );
    ...
    try {
      ...
      if (analysesVisible) {
        ...
        const zeroText = await page
          .locator('text=0 total, text=No analyses')
          .isVisible()
          .catch(() => false);
    ...
    try {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const analysisComplete = await page
          .locator('text=Analysis Complete, .analysis-complete')
          .isVisible()
          .catch(() => false);
        if (analysisComplete) {
          await reporter.recordResult(
      ...
      if (progressStuck) {
        await reporter.recordResult(
          'Progress Tracking',
          'FAIL',
          'Analysis stuck at Site Discovery'
        );
      } else {
    ...
    try {
      ...
      if (errorVisible) {
        ...
        await reporter.recordResult(
          ...
          'FAIL',
          'No error message for invalid URL'
        );
      }
      ...
      const emptyFieldError = await page
    ...
    try {
      ...
      if (consoleErrors.length === 0) {
        ...
        await reporter.recordResult(
          'Console Errors',
          'FAIL',
          `${consoleErrors.length} errors found`
        );
      }
    } catch (error) {
