...
test.describe('LLM.txt Mastery - Comprehensive QA Testing', () => {
    ...
    for (const link of headerLinks) {
      ...
      if (href && !href.startsWith('#') && !href.startsWith('mailto:')) {
        try {
          ...
          if (statusCode === 404) {
            reportBug('HIGH', 'Navigation', '404 Error on navigation link', 
              [`Click on "${text}" link in header`], 'Link should navigate to valid page', 
              `Got 404 error`, currentUrl);
          }
          ...
          reportBug('HIGH', 'Navigation', `Navigation link "${text}" failed`, 
            [`Click on "${text}" link`], 'Should navigate successfully', 
            `Error: ${error}`);
        }
      }
    }
    ...
    for (const link of footerLinks) {
      ...
      if (href && !href.startsWith('#') && !href.startsWith('mailto:')) {
        try {
          ...
          if (statusCode === 404) {
            reportBug('MEDIUM', 'Navigation', '404 Error on footer link', 
              [`Click on "${text}" link in footer`], 'Link should navigate to valid page', 
              `Got 404 error`);
          }
          ...
          reportBug('MEDIUM', 'Navigation', `Footer link "${text}" failed`, 
            [`Click on "${text}" link`], 'Should navigate successfully', 
            `Error: ${error}`);
        }
      }
  ...
  test('2. Free Tier Complete Flow - URL Entry to Download', async ({ page }) => {
    ...
    if (await urlInput.count() === 0) {
      reportBug('CRITICAL', 'Functionality', 'URL input field not found on homepage', 
        ['Navigate to homepage', 'Look for URL input field'], 
        'Should have visible URL input field', 'No URL input field found');
      return;
    }
    ...
    if (await analyzeButton.count() === 0) {
      reportBug('CRITICAL', 'Functionality', 'Analyze button not found', 
        ['Enter URL in input field', 'Look for analyze/submit button'], 
        'Should have analyze button', 'No analyze button found');
      return;
    }
    ...
    const tierSelection = page.locator('text="Free Tier", text="Coffee Tier", button:has-text("Free")');
    if (await tierSelection.count() > 0) {
      const freeTierButton = page.locator('button:has-text("Free"), button:has-text("Select Free")').first();
    ...
    if (await emailInput.count() > 0) {
      ...
      const submitEmailButton = page.locator('button:has-text("Continue"), button:has-text("Submit"), button[type="submit"]').first();
    ...
    while (!analysisComplete && attempts < maxAttempts) {
      ...
      if (await downloadButton.count() > 0 || await completeIndicator.count() > 0) {
        ...
        reportBug('HIGH', 'Functionality', 'Analysis failed with error', 
          ['Enter URL', 'Select free tier', 'Enter email', 'Wait for analysis'],
          'Analysis should complete successfully', `Got error: ${errorText}`);
        return;
      }
    }

    if (!analysisComplete) {
      reportBug('HIGH', 'Performance', 'Analysis taking too long or hanging', 
        ['Enter URL', 'Select free tier', 'Enter email', 'Wait for analysis'],
    ...
    if (await downloadButton.count() === 0) {
      reportBug('HIGH', 'Functionality', 'Download button not available after analysis', 
        ['Complete analysis flow'], 'Should have download button available', 
    ...
    if (!download) {
      reportBug('HIGH', 'Functionality', 'Download did not start', 
        ['Click download button'], 'Should start file download', 'No download initiated');
    } else {
      const filename = download.suggestedFilename();
      if (!filename.includes('.txt')) {
        reportBug('MEDIUM', 'Functionality', 'Downloaded file has wrong extension', 
          ['Download file'], 'Should download .txt file', `Downloaded: ${filename}`);
      }
    }
  });
  ...
  test('3. Coffee Tier Recognition and Login Prompt', async ({ page }) => {
    ...
    if (await emailInput.count() > 0) {
      ...
      if (await loginPrompt.count() === 0 && await dashboardRedirect.count() === 0) {
        reportBug('MEDIUM', 'Functionality', 'Coffee tier email not recognized', 
          ['Enter coffee tier email'], 'Should prompt for login or redirect to dashboard',
  ...
  test('4. Mobile Responsiveness - 375px Width', async ({ page }) => {
    ...
    if (await navLinks.count() > 0) {
      ...
      if (!isVisible && await mobileMenu.count() === 0) {
        reportBug('HIGH', 'UI', 'Navigation not accessible on mobile', 
          ['Resize to 375px width'], 'Should have visible nav or mobile menu', 
    ...
    if (await urlInput.count() > 0) {
      ...
      if (boundingBox && boundingBox.width < 200) {
        reportBug('MEDIUM', 'UI', 'URL input too narrow on mobile', 
          ['Resize to mobile width', 'Check URL input field'], 
          'Input should be adequately sized', `Input width: ${boundingBox.width}px`);
      }
    }
    ...
    for (let i = 0; i < Math.min(buttonCount, 5); i++) {
      ...
      if (boundingBox && (boundingBox.height < 44 || boundingBox.width < 44)) {
        ...
        reportBug('MEDIUM', 'UI', `Button too small for mobile touch: "${buttonText}"`, 
          ['Check button size on mobile'], 'Buttons should be minimum 44px touch target',
  ...
  test('5. Error Handling - Invalid URLs and Network Errors', async ({ page }) => {
      ...
      if (await urlInput.count() > 0) {
        ...
        if (await analyzeButton.count() > 0) {
          ...
          if (await errorMessage.count() === 0) {
            reportBug('MEDIUM', 'Functionality', `No error shown for invalid URL: ${invalidUrl}`, 
              [`Enter invalid URL: ${invalidUrl}`, 'Click analyze'], 
              'Should show error message', 'No error handling visible');
          }
        }
    ...
    if (await analyzeButton.count() > 0) {
      ...
      if (await validationError.count() === 0) {
        reportBug('LOW', 'Functionality', 'No validation error for empty form', 
          ['Click analyze without entering URL'], 'Should show validation error', 
  ...
  test('6. Pricing Page and Tier Information', async ({ page }) => {
    ...
    const pricingLink = page.locator('a:has-text("Pricing"), a:has-text("Plans"), a[href*="pricing"]').first();
    ...
    if (await pricingLink.count() > 0) {
      ...
      if (await tiers.count() === 0) {
        reportBug('HIGH', 'Functionality', 'No pricing tiers visible on pricing page', 
          ['Navigate to pricing page'], 'Should show pricing tiers', 
      ...
      if (await priceElements.count() === 0) {
        reportBug('HIGH', 'Functionality', 'No price information visible', 
          ['Check pricing page content'], 'Should display prices', 
          'No pricing details found');
      }
      
      // Check for feature comparisons
      const features = page.locator('text="feature", text="included", text="unlimited", text="limited"');
      if (await features.count() < 3) {
        reportBug('MEDIUM', 'UI', 'Insufficient feature information on pricing page', 
          ['Review pricing page features'], 'Should clearly show tier differences', 
          'Limited feature comparison available');
      }
    } else {
      reportBug('MEDIUM', 'Navigation', 'Pricing page link not found', 
        ['Look for pricing link in navigation'], 'Should have accessible pricing page', 
  ...
  test('7. Form Validation and Input Handling', async ({ page }) => {
    ...
    if (await urlInput.count() > 0) {
      ...
      if (await analyzeButton.count() > 0) {
        ...
        if (isValid) {
          reportBug('LOW', 'Functionality', 'Form submits without required URL', 
            ['Submit form without URL'], 'Should prevent submission', 
          ...
          if (await emailInput.count() > 0) {
            ...
            if (await submitButton.count() > 0) {
              ...
              const emailValid = await page.evaluate(() => {
                ...
                return Array.from(emailInputs).every(input => (input as HTMLInputElement).checkValidity());
              });
              
              if (emailValid) {
                reportBug('LOW', 'Functionality', 'Invalid email accepted', 
  ...
  test('8. Payment/Stripe Integration Buttons', async ({ page }) => {
    ...
    const upgradeButtons = page.locator('button:has-text("Upgrade"), button:has-text("Buy"), a:has-text("Get Started"), button:has-text("Coffee")');
      ...
      for (let i = 0; i < Math.min(upgradeCount, 3); i++) {
        ...
        try {
          ...
          if (currentUrl.includes('stripe') || await stripeIndicator.count() > 0) {
            ...
            reportBug('LOW', 'Functionality', `Payment button "${buttonText}" may not lead to payment`, 
              [`Click "${buttonText}" button`], 'Should lead to payment system', 
              `Led to: ${currentUrl}`);
          }
          ...
          reportBug('MEDIUM', 'Functionality', `Payment button "${buttonText}" not clickable`, 
            [`Click "${buttonText}" button`], 'Button should be clickable', 
            `Error: ${error}`);
        }
      }
    }
  ...
  test('9. Performance and Loading Times', async ({ page }) => {
    ...
    if (loadTime > 5000) {
      reportBug('MEDIUM', 'Performance', 'Page load time too slow', 
        ['Navigate to homepage'], 'Should load within 5 seconds', 
    ...
    if (imageCount > 0) {
      for (let i = 0; i < Math.min(imageCount, 5); i++) {
        ...
        if (naturalWidth === 0) {
          ...
          reportBug('LOW', 'Performance', `Image failed to load: ${src}`, 
            ['Check page images'], 'All images should load', 
