...
test.describe('Conversion Diagnostic Tests', () => {
  test('should inspect homepage structure and tier selection', async ({ page }) => {
    ...
    if (!emailCaptureVisible) {
      ...
      if (urlInputVisible) {
        console.log('ℹ️  User appears to be authenticated - seeing URL input instead of email capture');
        return;
      }
    }
  ...
  test('should test tier selection interaction', async ({ page }) => {
    ...
    const radioGroups = await page.locator('[role="radiogroup"], .radio-group, [data-testid*="radio"]').all();
