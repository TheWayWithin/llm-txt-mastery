...
async function capturePageDiagnostics(page: Page, testName: string) {
  ...
  const isLoading = await page.locator('text=/loading|please wait/i, [class*="loading"], [class*="spinner"]').isVisible();
  if (isLoading) {
...
test.describe('Password Reset - Diagnostic Tests', () => {
  ...
  test('Diagnose forgot password page', async ({ page }) => {
    ...
    try {
      ...
      const hasPasswordResetText = await page.locator('text=/reset.*password|forgot.*password/i').isVisible();
  ...
  test('Diagnose reset password page with token', async ({ page }) => {
    ...
    try {
      ...
      // Check for specific text content
      const hasNewPasswordText = await page.locator('text=/set.*new.*password|new.*password/i').isVisible();
  ...
  test('Check site accessibility and routing', async ({ page }) => {
    ...
    try {
      ...
      try {
        const forgotPasswordLink = page.locator('a[href*="forgot-password"], a:has-text("forgot password")');
      ...
      const apiHealth = await page.request.get('https://llm-txt-mastery-production.up.railway.app/api/health');
      console.log(`🔗 API health status: ${apiHealth.status()}`);
      
      const forgotPasswordApi = await page.request.post('https://llm-txt-mastery-production.up.railway.app/api/auth/request-password-reset', {
