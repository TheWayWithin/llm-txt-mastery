  ...
  test.describe('🆕 NEW USER SIGNUP FLOWS', () => {
    test('Growth Tier Signup → Stripe Checkout Flow', async ({ page }) => {
      ...
      try {
        ...
        const checkoutRequestPromise = page.waitForRequest(req => 
          req.url().includes('/api/stripe/create-growth-checkout') &&
          req.method() === 'POST'
        );
        ...
        await page.waitForSelector('form, .StripeElement, iframe, [data-testid="hosted-payment-page"]', { 
          timeout: 15000 
        });
    ...
    test('Scale Tier Signup → Stripe Checkout Flow', async ({ page }) => {
      ...
      try {
        ...
        // Intercept network requests to validate API calls
        const checkoutRequestPromise = page.waitForRequest(req => 
          req.url().includes('/api/stripe/create-scale-checkout') &&
          req.method() === 'POST'
        );
    ...
    test('Coffee → Growth Tier Upgrade Flow', async ({ page }) => {
      ...
      try {
        ...
        if (upgradeVisible) {
          ...
          const upgradeRequestPromise = page.waitForRequest(req => 
          ...
          if (await manageSubscription.count() > 0) {
            ...
            const postNavUpgrade = page.locator('button:has-text("Upgrade"), a:has-text("Upgrade")');
            const postNavVisible = await postNavUpgrade.count() > 0;
            console.log(`Upgrade options after navigation: ${postNavVisible ? 'Found' : 'Not found'}`);
          }
        }
      ...
      try {
        ...
        for (const cancelPage of cancellationPages) {
          ...
          if (hasMessage) {
            ...
            console.log(`${hasRetry ? '✅' : '⚠️ '} Retry option ${hasRetry ? 'available' : 'missing'} on ${cancelPage}`);
    ...
    test('Subscription Success Page Integration', async ({ page }) => {
      ...
      try {
        ...
        for (const tier of testTiers) {
          ...
          console.log(`${hasSuccess ? '✅' : '❌'} Success message for ${tier} tier: ${hasSuccess ? 'shown' : 'missing'}`);
          ...
          console.log(`${hasTierInfo ? '✅' : '⚠️ '} Tier-specific info for ${tier}: ${hasTierInfo ? 'shown' : 'missing'}`);
          ...
          console.log(`${hasCta ? '✅' : '❌'} CTA button for ${tier} tier: ${hasCta ? 'present' : 'missing'}`);
        }
