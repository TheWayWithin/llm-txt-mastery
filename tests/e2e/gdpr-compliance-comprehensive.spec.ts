...
class GDPRTestHelper {
  ...
  getConsoleLogs(filter?: string): string[] {
    return filter ? this.consoleLogs.filter((log) => log.includes(filter)) : this.consoleLogs;
  }
...
test.describe('GDPR Compliance Comprehensive Testing Suite', () => {
  ...
  test('GDPR-002: Accept All cookies functionality', async ({ page }) => {
    ...
    console.log(
      `🍪 Cookie counts - Necessary: ${cookiesByCategory.necessary.length}, Analytics: ${cookiesByCategory.analytics.length}`
    );
  ...
  test('GDPR-003: Reject Optional cookies functionality', async ({ page }) => {
    ...
    const cookiesByCategory = await helper.getCookiesByCategory();
    console.log(
      `🍪 Cookie counts after rejection - Necessary: ${cookiesByCategory.necessary.length}, Analytics: ${cookiesByCategory.analytics.length}, Marketing: ${cookiesByCategory.marketing.length}`
    );

    // Verify analytics tracking is blocked
    expect(
      cookiesByCategory.analytics.length,
      'Analytics cookies should be minimal when rejected'
    ).toBeLessThanOrEqual(1);
    expect(
      cookiesByCategory.marketing.length,
      'Marketing cookies should be blocked when rejected'
    ).toBeLessThanOrEqual(1);
  ...
  test('GDPR-004: GTM Consent Mode integration validation', async ({ page }) => {
    ...
    for (const state of testStates) {
      ...
      const gtmRequests = helper.getNetworkRequests(
        (req) =>
          req.url.includes('googletagmanager.com') &&
  ...
  test('GDPR-005: Cookie categorization validation', async ({ page }) => {
    ...
    // Basic validation
    expect(
      cookiesByCategory.necessary.length,
      'Should have some necessary cookies'
  ...
  test('GDPR-006: Privacy policy and cookie policy link validation', async ({ page }) => {
    ...
    if (await privacyLink.isVisible()) {
      ...
      expect(
        currentUrl.includes('privacy') || currentUrl.includes('policy'),
    ...
    if (await cookieLink.isVisible()) {
      ...
      const currentUrl = page.url();
      expect(
        currentUrl.includes('cookie') || currentUrl.includes('policy'),
  ...
  test('GDPR-007: Consent preferences persistence across sessions', async ({ page, context }) => {
    ...
    expect(
      newSessionCookies.analytics.length,
      'Analytics cookies should remain minimal in new session'
    ).toBeLessThanOrEqual(initialCookies.analytics.length + 1);
  ...
  test('GDPR-009: Cross-browser consent behavior validation', async ({ page, browserName }) => {
    ...
    if (bannerVisible) {
      ...
      console.log(
        `${browserName} - Cookies: N:${cookiesByCategory.necessary.length}, A:${cookiesByCategory.analytics.length}, M:${cookiesByCategory.marketing.length}`
      );
  ...
  test('GDPR-010: Performance impact of GDPR implementation', async ({ page }) => {
    ...
    expect(
      bannerLoadTime,
      'Page should load with consent banner within reasonable time'
    ).toBeLessThan(10000);
  ...
  test('GDPR-012: Compliance documentation generation', async ({ page }) => {
    ...
    console.log('\n🍪 COOKIE ANALYSIS:');
    console.log(
      `Before Consent - Total: ${Object.values(complianceReport.cookies.beforeConsent).reduce((a: any, b: any) => a + b.length, 0)}`
    );
    console.log(
      `After Accept - Total: ${Object.values(complianceReport.cookies.afterAccept).reduce((a: any, b: any) => a + b.length, 0)}`
    );
    console.log(
      `After Reject - Total: ${Object.values(complianceReport.cookies.afterReject).reduce((a: any, b: any) => a + b.length, 0)}`
    );

    // Overall compliance assessment
    const passedTests = Object.values(complianceReport.tests).filter(
      (test) => test === true
    ).length;
    ...
    expect(compliancePercentage, 'GDPR compliance should be at least 60%').toBeGreaterThanOrEqual(
      60
    );
