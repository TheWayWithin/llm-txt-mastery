...
test.describe('Connection Pool Regression Tests', () => {
  ...
  test.describe('Free Tier Functionality', () => {
    test('Free tier analysis still respects 20-page limit with connection pooling', async ({ page }) => {
      ...
      const pageCountElement = await page.locator('[data-testid="page-count"], .page-count').first();
    ...
    test('Free tier shows appropriate messaging about connection pooling benefits', async ({ page }) => {
      const email = generateTempEmail();
    ...
    test('Free tier handles connection pooling errors gracefully', async ({ page }) => {
      ...
      const errorOrComplete = await Promise.race([
        ...
        page.waitForSelector('[data-testid="analysis-complete"], text="Analysis Complete"', { timeout: 30000 })
      ]);
  ...
  test.describe('Coffee Tier Functionality', () => {
    test('Coffee tier gets 200 pages with connection pooling enabled', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="user-dashboard"], .user-dashboard', { timeout: 10000 });
      ...
      console.log(`Coffee tier analysis found ${pageCount} pages in ${duration}ms with connection pooling`);
    });
  ...
  test.describe('Bot Protection and Resilience', () => {
    test('Bot protection detection still works with connection pooling', async ({ page }) => {
      ...
      const result = await Promise.race([
        page.waitForSelector('[data-testid="analysis-complete"], text="Analysis Complete"', { timeout: 60000 }),
        page.waitForSelector('[data-testid="bot-protection"], text="Bot protection detected"', { timeout: 60000 }),
      ...
      const hasResults = await page.isVisible('[data-testid="analysis-results"], .analysis-results');
      const hasError = await page.isVisible('[data-testid="error-message"], .error-message');
      const hasBotProtection = await page.isVisible('[data-testid="bot-protection"], text="Bot protection"');
    ...
    test('Connection pooling handles consecutive failures correctly', async ({ page }) => {
      ...
      await page.fill('[data-testid="url-input"], input[name="url"]', 'https://httpstat.us/timeout');
  ...
  test.describe('Edge Cases and Error Handling', () => {
    ...
    test('Handles concurrent analyses correctly', async ({ page }) => {
      ...
      const analyzeButton = page.locator('[data-testid="analyze-button"], button:has-text("Analyzing")');
