...
test.describe('Production Manual Validation', () => {
  test('Step 1: Homepage and signup navigation', async ({ page }) => {
    ...
    const signupButton = page.locator(
      'a[href*="/signup"], button:has-text("Sign up"), a:has-text("Sign up"), button:has-text("Get Started"), a:has-text("Get Started")'
    );
  ...
  test('Step 2: Signup form validation', async ({ page }) => {
    ...
    const submitButton = page.locator(
      'button[type="submit"], button:has-text("Sign up"), button:has-text("Create"), input[type="submit"]'
    );
  ...
  test('Step 3: Check-email page validation', async ({ page }) => {
    ...
    const submitButton = page
      .locator('button[type="submit"], button:has-text("Sign up"), button:has-text("Create")')
      .first();
    ...
    if (
      ...
      if (
        ...
        console.log(
          '❌ INCORRECT: Redirected directly to analyze page (should go to check-email first)'
        );
      } else {
  ...
  test('Step 4: Analyze page access validation', async ({ page }) => {
    ...
    const hasAnalyzeFeatures =
      pageContent?.toLowerCase().includes('url') && pageContent?.toLowerCase().includes('analyze');
    ...
    if (hasAnalyzeFeatures) {
      console.log(
        'ℹ️ Analyze page accessible (may indicate user is logged in or auth not required)'
      );
  ...
  test('Step 6: Complete flow verification', async ({ page }) => {
    ...
    const steps = [
      ...
      {
        name: 'Signup',
        url: 'https://www.llmtxtmastery.com/signup',
        screenshot: 'step6-signup.png',
      },
      { name: 'Login', url: 'https://www.llmtxtmastery.com/login', screenshot: 'step6-login.png' },
      {
        name: 'Analyze',
        url: 'https://www.llmtxtmastery.com/analyze',
        screenshot: 'step6-analyze.png',
      },
      {
        name: 'Check Email',
        url: 'https://www.llmtxtmastery.com/check-email',
        screenshot: 'step6-check-email.png',
      },
    ];
