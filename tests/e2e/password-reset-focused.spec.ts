...
test.describe('Password Reset - Phase 1: Core Functionality', () => {
  ...
  test('Forgot password form submission works', async ({ page }) => {
    ...
    await expect(page.locator('text=/check.*email|sent.*link|if.*account.*exists/i')).toBeVisible({ timeout: 10000 });
...
test.describe('Password Reset - Phase 2: Security Features', () => {
  ...
  test('Password strength validation', async ({ page }) => {
    ...
    for (const { password, description } of weakPasswords) {
      ...
      const hasValidationMessage = await page.locator('text=/password.*must|at.*least|contains.*at.*least/i').isVisible();
...
test.describe('Password Reset - Phase 3: Error Handling', () => {
  ...
  test('Network error simulation', async ({ page }) => {
    ...
    await expect(page.locator('text=/network.*error|connection.*error|try.*again/i')).toBeVisible({ timeout: 10000 });
  ...
  test('Loading states and button behavior', async ({ page }) => {
    ...
    const hasLoadingState = await page.locator('text=/sending|loading/i, [class*="spin"], [class*="loading"]').isVisible();
...
test.describe('Password Reset - Phase 4: Compatibility', () => {
  ...
  test('Responsive design validation', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      console.log(`✅ Responsive design works at ${viewport.width}x${viewport.height} (${viewport.name})`);
    }
...
test.describe('Password Reset - Phase 5: API Integration', () => {
  ...
  test('API endpoint accessibility', async ({ page }) => {
    ...
    const forgotPasswordResponse = await page.request.post(`${API_BASE_URL}/api/auth/request-password-reset`, {
    ...
    console.log(`✅ Forgot password API endpoint accessible (status: ${forgotPasswordResponse.status()})`);
    
    // Test reset password endpoint
    const resetPasswordResponse = await page.request.post(`${API_BASE_URL}/api/auth/reset-password`, {
      data: { 
        token: 'test-token',
    ...
    console.log(`✅ Reset password API endpoint accessible (status: ${resetPasswordResponse.status()})`);
  });
  ...
  test('API error response handling', async ({ page }) => {
    ...
    const invalidResponse = await page.request.post(`${API_BASE_URL}/api/auth/request-password-reset`, {
...
test.describe('Password Reset - Test Summary', () => {
  ...
  test('Generate focused test report', async ({ page }) => {
    ...
    await page.goto('data:text/html,<h1>Password Reset Tests Complete</h1><p>Focused testing completed successfully!</p>');
