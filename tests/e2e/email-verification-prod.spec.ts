...
test.describe('Email Verification Production Test', () => {
  ...
  test('should register and check for email verification banner', async ({ page }) => {
    ...
    page.on('console', async (msg) => {
      ...
      if (
        text.includes('User verification check') ||
    ...
    if (!clicked) {
      ...
      await page
        .locator('button')
        .filter({ hasText: /sign|log|user/i })
        .first()
        .click();
    }
    ...
    for (const log of consoleLogs) {
      if (
        log.text.includes('User verification check') ||
        log.text.includes('EmailVerificationBanner')
      ) {
    ...
    const verificationCheckLog = consoleLogs.find((log) =>
      log.text.includes('User verification check')
    );
