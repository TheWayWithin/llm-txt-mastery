  ...
  for (const pattern of targetPatterns) {
    ...
    for (let i = 0; i < Math.min(elements.length, 3); i++) {
      ...
      try {
        ...
        if (textContent && textContent.length > 50 && textContent.length < 500 && 
            textContent.includes('example.com') && textContent.includes('llms.txt')) {
  ...
  for (const selector of codeStyleSelectors) {
    try {
      ...
      for (const element of elements) {
        const textContent = await element.textContent();
        if (textContent && textContent.includes('example.com llms.txt') && textContent.length > 50 && textContent.length < 1000) {
          const tagName = await element.evaluate(el => el.tagName);
          const className = await element.evaluate(el => el.className);
