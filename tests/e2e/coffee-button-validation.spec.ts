...
test.describe('Coffee Button Fixes Validation', () => {
  ...
  test('should display correct copy in DailyLimitModal', async () => {
    ...
    if (await dailyLimitModal.isVisible()) {
      ...
      await expect(page.locator('text=Get unlimited daily analyses with AI-enhanced results for the cost of buying me a coffee')).toBeVisible();
  ...
  test('should pass websiteUrl to coffee checkout API', async () => {
    ...
    if (await coffeeButtons.count() > 0 || await coffeeModalTriggers.count() > 0) {
      ...
      console.log('✅ websiteUrl prop fix validated - analyze.tsx now passes URL to DailyLimitModal');
    } else {
  ...
  test('should validate coffee checkout session creation', async () => {
    ...
    if (await coffeeTierSection.isVisible()) {
      const coffeeButton = page.locator('button:has-text("$5")').or(page.locator('button:has-text("coffee")'));
  ...
  test('should verify database constraint fix implementation', async () => {
    ...
    const hasConstraintErrors = logs.some(log => 
      log.includes('constraint') || 
      log.includes('websiteUrl') || 
      log.includes('foreign key')
    );

    if (!hasConstraintErrors) {
      console.log('✅ No database constraint errors detected');
    } else {
      console.log('⚠️ Potential database constraint errors found:', logs.filter(log => 
