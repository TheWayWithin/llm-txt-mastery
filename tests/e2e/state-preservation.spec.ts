...
async function captureCurrentState(page: Page): Promise<StateSnapshot> {
  const snapshot: StateSnapshot = {
    ...
    urlInputValue: await page
      .locator(SELECTORS.urlInput)
      .inputValue()
      .catch(() => ''),
    usageText: await page
      .locator(SELECTORS.usageCounter)
      .textContent()
      .catch(() => null),
    tierText: await page
      .locator(SELECTORS.tierBadge)
      .textContent()
      .catch(() => null),
    isEmailCaptureVisible: await page
      .locator(SELECTORS.emailCaptureForm)
      .isVisible()
      .catch(() => false),
    isTierLimitsVisible: await page
      .locator(SELECTORS.tierLimitsDisplay)
      .isVisible()
      .catch(() => false),
    isContentReviewVisible: await page
      .locator(SELECTORS.contentReviewCard)
      .isVisible()
      .catch(() => false),
    isFileGenerationVisible: await page
      .locator(SELECTORS.fileGenerationCard)
      .isVisible()
      .catch(() => false),
    hasErrors: await page
      .locator(SELECTORS.errorDisplay)
      .isVisible()
      .catch(() => false),
    timestamp: Date.now(),
  };
...
async function completeFullAnalysis(
  page: Page,
  url: string,
...
test.describe('State Preservation - Core Context Validation', () => {
  ...
  test('should preserve usage tracking accuracy across multiple resets', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i < 2) {
        // Don't reset after last iteration
        // Perform smart reset
        await page.click(SELECTORS.analyzeAnotherButton);
        await expect(page.locator(SELECTORS.urlInput)).toBeVisible({ timeout: 5000 });
...
test.describe('State Preservation - Component Synchronization', () => {
  ...
  test('should handle component state during rapid resets', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i === 2) {
        // Only complete analysis on last iteration
        await page.fill(SELECTORS.urlInput, TEST_URLS[1]);
        await page.click(SELECTORS.analyzeButton);
...
test.describe('State Preservation - Edge Cases and Error Recovery', () => {
  ...
  test('should maintain state consistency during error conditions', async ({ page }) => {
    ...
    const resetButton = page
      .locator(SELECTORS.analyzeAnotherButton)
      .or(
        page
          .locator('button:has-text("Try Again")')
          .or(page.locator('button:has-text("Start Over")'))
      );
