...
test.describe('Email Verification System', () => {
  ...
  test('should show email verification banner after registration', async ({ page }) => {
    ...
    page.on('console', msg => {
      if (msg.text().includes('User verification check') || 
    ...
    const welcomeText = await page.locator('text=/Welcome.*' + testEmail.split('@')[0] + '/i').isVisible();
    ...
    for (const text of bannerTexts) {
      const isVisible = await page.locator(`text=/${text}/i`).isVisible().catch(() => false);
      if (isVisible) {
    ...
    if (!bannerFound) {
      ...
      const alertExists = await page.locator('.border-amber-200, .bg-amber-50, [role="alert"]').isVisible().catch(() => false);
  ...
  test('should test with production URL', async ({ page }) => {
    ...
    // Check for banner
    const bannerVisible = await page.locator('text=/verify.*email/i').isVisible().catch(() => false);
