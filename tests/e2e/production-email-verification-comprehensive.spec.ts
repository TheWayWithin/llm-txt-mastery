...
test.describe('Production Email Verification Comprehensive Tests', () => {
  ...
  test('COMPLETE EMAIL FLOW: Signup → Check-Email → Verification → Analyze', async ({ page }) => {
    ...
    const resendButton = page.locator('button:has-text("Resend"), a:has-text("Resend"), button:has-text("Send again"), a:has-text("Send again")');
    
    if (await resendButton.isVisible()) {
      await resendButton.click();
      
      // Look for confirmation message
      const confirmationVisible = await page.locator('text=sent, text=resent, text=delivered').isVisible({ timeout: 5000 });
    ...
    const isBlocked = currentUrl.includes('/check-email') || 
                     await page.locator('text=verify your email, text=check your email').isVisible({ timeout: 5000 });
  ...
  test('URL preservation through verification process', async ({ page }) => {
    ...
    if (isRedirected) {
      ...
      const hasPendingUrl = prefilledValue === pendingUrl ||
  ...
  test('Check-email page content validation', async ({ page }) => {
    ...
    const hasTitle = await page.locator('h1, h2, h3').filter({ hasText: /check.*email/i }).isVisible();
    ...
    const hasInstructions = await page.locator('text=verification, text=link, text=click').isVisible();
  ...
  test('Browser back/forward navigation through verification flow', async ({ page }) => {
    ...
    const isValidBack = backUrl.includes('/check-email') || 
                       backUrl.includes('/signup') ||
                       backUrl.includes('/');
