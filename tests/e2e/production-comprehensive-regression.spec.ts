...
test.describe('🔍 PRODUCTION REGRESSION VALIDATION', () => {
  ...
  test('🔒 GDPR Compliance Validation', async ({ page }) => {
    ...
    for (const selector of consentSelectors) {
      if (await freshPage.locator(selector).first().isVisible().catch(() => false)) {
    ...
    const enzuzoScript = await freshPage.evaluate(() => {
      return document.querySelector('script[src*="enzuzo"]') !== null ||
  ...
  test('🔗 Critical Route Accessibility', async ({ page }) => {
    ...
    for (const route of criticalRoutes) {
      try {
        ...
        const is404 = pageTitle.includes('404') || 
                      await page.locator('text=404, text=Not Found').isVisible().catch(() => false);
    ...
    console.log(`🎯 Route Accessibility: ${successRate.toFixed(1)}% (${passedRoutes}/${totalRoutes})`);
  });
});