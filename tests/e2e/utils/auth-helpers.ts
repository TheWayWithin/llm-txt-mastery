...
export class AuthTestHelper {
  ...
  async fillSignupForm(email: string, password: string): Promise<void> {
    ...
    const confirmPasswordField = this.page.locator(
      'input[name="confirmPassword"], input[name="confirm-password"], input[type="password"]:nth-of-type(2)'
    );
  ...
  async waitForAuthCompletion(expectedUrl: string = '/analyze'): Promise<void> {
    try {
      ...
      throw new Error(
        `Authentication did not complete successfully. Expected: ${expectedUrl}, Got: ${currentUrl}`
      );
    }
  }
...
export function hasAuthResolutionMessages(messages: string[]): {
  ...
  return {
    hasAuthResolved: messages.some(
      (msg) => msg.includes('Auth resolved') || msg.includes('AUTH_RESOLVED')
    ),
    hasCoffeeMessages: messages.some((msg) => msg.includes('Coffee tier') || msg.includes('☕')),
    hasStateTransitions: messages.some(
      (msg) => msg.includes('State transition') || msg.includes('🔄')
    ),
  };
}
...
export async function performValidatedSmartReset(page: Page): Promise<{
  ...
  const beforeUsage = await page
    .locator('text=/\\d+ \\/ \\d+/')
    .textContent()
    .catch(() => null);
  const beforeTier = await page
    .locator('span:has-text("Starter"), span:has-text("Coffee")')
    .textContent()
    .catch(() => null);
  ...
  const afterUsage = await page
    .locator('text=/\\d+ \\/ \\d+/')
    .textContent()
    .catch(() => null);
  const afterTier = await page
    .locator('span:has-text("Starter"), span:has-text("Coffee")')
    .textContent()
    .catch(() => null);
  ...
  console.log(
    `✅ Smart reset validated - Usage: ${beforeUsage} → ${afterUsage}, Tier: ${beforeTier} → ${afterTier}`
  );
...
export async function completeAnalysisForSmartReset(
  ...
  await expect(page.locator('button:has-text("Proceed with Analysis")')).toBeVisible({
    timeout: 15000,
  });
  await page.click('button:has-text("Proceed with Analysis")');

  // Wait for analysis completion
  await expect(page.locator('[class*="card"]:has-text("Content Review")')).toBeVisible({
    timeout: 60000,
  });
  ...
  if (generateFile) {
    ...
    await expect(
      page.locator('[class*="card"]:has-text("File Generated Successfully")')
...
export async function validateSmartResetBehavior(page: Page): Promise<void> {
  ...
  const hasUsageDisplay = await page
    .locator('text=/\\d+ \\/ \\d+/')
    .isVisible()
    .catch(() => false);
  const hasTierBadge = await page
    .locator('span:has-text("Starter"), span:has-text("Coffee")')
    .isVisible()
    .catch(() => false);
  ...
  const emailCaptureVisible = await page
