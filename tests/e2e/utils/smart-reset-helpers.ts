...
export async function captureStateSnapshot(page: Page): Promise<SmartResetStateSnapshot> {
  const snapshot: SmartResetStateSnapshot = {
    url: page.url(),
    urlInputValue: await page
      .locator(SMART_RESET_SELECTORS.urlInput)
      .inputValue()
      .catch(() => ''),
    usageText: await page
      .locator(SMART_RESET_SELECTORS.usageCounter)
      .textContent()
      .catch(() => null),
    tierText: await page
      .locator(SMART_RESET_SELECTORS.tierBadge)
      .textContent()
      .catch(() => null),
    isEmailCaptureVisible: await page
      .locator(SMART_RESET_SELECTORS.emailCaptureForm)
      .isVisible()
      .catch(() => false),
    isTierLimitsVisible: await page
      .locator(SMART_RESET_SELECTORS.tierLimitsDisplay)
      .isVisible()
      .catch(() => false),
    isContentReviewVisible: await page
      .locator(SMART_RESET_SELECTORS.contentReviewCard)
      .isVisible()
      .catch(() => false),
    isFileGenerationVisible: await page
      .locator(SMART_RESET_SELECTORS.fileGenerationCard)
      .isVisible()
      .catch(() => false),
    hasErrors: await page
      .locator(SMART_RESET_SELECTORS.errorDisplay)
      .isVisible()
      .catch(() => false),
    hasUpgradePrompts: await page
      .locator(SMART_RESET_SELECTORS.upgradePrompts)
      .isVisible()
      .catch(() => false),
    timestamp: Date.now(),
  };
...
export async function completeAnalysisFlow(
  ...
  if (requiresEmail) {
    await expect(page.locator(SMART_RESET_SELECTORS.emailCaptureForm)).toBeVisible({
      timeout: 10000,
    });
  ...
  await expect(page.locator(SMART_RESET_SELECTORS.contentReviewCard)).toBeVisible({
    timeout: 60000,
  });
 ...
 */
export async function validateAnalysisDataCleared(
  afterState: SmartResetStateSnapshot
): Promise<void> {
...
export async function verifyCoffeeTierBehavior(page: Page): Promise<void> {
  ...
  await expect(page.locator('text=/premium credits/i, text=/credit/i')).toBeVisible({
    timeout: 5000,
  });
...
export async function monitorSmartResetTransitions(page: Page): Promise<string[]> {
  ...
  page.on('console', (msg) => {
    const text = msg.text();
    if (
      text.includes('START_NEW_ANALYSIS') ||
...
export function validateSmartResetTransitions(transitions: string[]): void {
  ...
  const hasUserContextPreservation = transitions.some(
    (t) => t.includes('Preserving user context') || t.includes('preserved')
  );
 ...
 */
export async function testRapidSmartResets(page: Page, resetCount: number = 5): Promise<void> {
