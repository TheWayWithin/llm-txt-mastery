...
test.describe('Production Regression Test Suite', () => {
  ...
  test.describe('FREE TIER TESTING', () => {
    test('should handle complete free tier user flow', async ({ page, browser }) => {
      ...
      // Start analysis flow
      await page.fill(
        '[data-testid="url-input"], input[placeholder*="website"], input[type="url"]',
      ...
      // Select FREE tier
      await page.click(
        '[data-testid="tier-free"], button:has-text("Free"), .tier-card:has-text("Free")'
      );

      // Submit
      await page.click(
        'button:has-text("Continue"), button:has-text("Start Analysis"), button[type="submit"]'
      );

      // Wait for analysis to start
      await page.waitForSelector('.progress, [data-testid="analysis-progress"], .analysis-status', {
        timeout: 30000,
      });
      ...
      await page.waitForSelector(
        '[data-testid="analysis-complete"], .analysis-results, .page-count',
        { timeout: 120000 }
      );
      ...
      const analyzeAnotherBtn = page.locator(
        'button:has-text("Analyze Another"), a:has-text("Analyze Another")'
      );
    ...
    test('should enforce daily limits for free users', async ({ page, browser }) => {
      ...
      const hasUsageDisplay = await page
        .locator('.usage-count, [data-testid="usage-count"]')
        .isVisible();
  ...
  test.describe('COFFEE TIER TESTING', () => {
    test('should load Stripe checkout for coffee tier', async ({ page, browser }) => {
      ...
      // Select COFFEE tier
      await page.click(
        '[data-testid="tier-coffee"], button:has-text("Coffee"), .tier-card:has-text("Coffee")'
      );
      ...
      try {
        ...
        await expect(
          stripeFrame.locator('input[placeholder*="card"], input[placeholder*="Card"]')
        ...
        if (
          ...
          (await page.locator('.stripe-checkout, #stripe-element').isVisible())
        ) {
  ...
  test.describe('PERFORMANCE TESTING', () => {
    test('should meet performance benchmarks', async ({ page }) => {
      ...
      const testSites = ['https://docs.python.org', 'https://reactjs.org', 'https://nodejs.org'];
    ...
    test('should handle invalid URLs gracefully', async ({ page }) => {
      ...
      for (const invalidUrl of invalidUrls) {
        ...
        const hasError = await page
          .locator('.error, .alert-destructive, [role="alert"]')
          .isVisible({ timeout: 5000 });
        const hasValidation = await page
          .locator('input:invalid, .error-message')
          .isVisible({ timeout: 5000 });

        expect(hasError || hasValidation).toBe(true);
        console.log(
          `✅ Error handling for ${invalidUrl}: ${hasError ? 'Error shown' : 'Validation shown'}`
        );
