...
test.describe('Verification Flow Improvements', () => {
  ...
  test('Authenticated users see URL input immediately on home page', async ({ page }) => {
    ...
    await expect(
      urlInputSection.locator('button:has-text("Start New Analysis")')
    ).not.toBeVisible();
  ...
  test('No extra "Start New Analysis" buttons needed for all user tiers', async ({ page }) => {
    ...
    for (const userType of userTiers) {
      ...
      await expect(
        page.locator(`text=Welcome back, ${userType.email.split('@')[0]}!`)
      ).toBeVisible();
      ...
      const hasStartButton = await mainFlowArea
        .locator('button:has-text("Start New Analysis")')
        .count();

      if (hasStartButton > 0) {
        throw new Error(
          `${userType.tier} tier should not require "Start New Analysis" button in main flow - should see URL input directly`
        );
      }
