...
test.describe('Password Reset Diagnostic - Site Structure', () => {
  ...
  test('Analyze forgot password page structure', async ({ page }) => {
    ...
    if (pageUrl === `${PRODUCTION_URL}/` || pageUrl === `${PRODUCTION_URL}`) {
      ...
      const authModal = page.locator('[data-testid*="auth"], [data-testid*="login"], .auth-modal, .login-modal');
  ...
  test('API endpoint validation', async ({ page }) => {
    ...
    try {
      const forgotResponse = await page.request.post(`${API_BASE_URL}/api/auth/request-password-reset`, {
...
test.describe('Password Reset Diagnostic - Authentication Analysis', () => {
  ...
  test('Analyze current authentication system', async ({ page }) => {
    ...
    const loginButtons = page.locator('button:has-text("Login"), a:has-text("Login"), [data-testid*="login"]');
    const signupButtons = page.locator('button:has-text("Sign"), a:has-text("Sign"), [data-testid*="signup"]');
...
test.describe('Password Reset Diagnostic - Security Analysis', () => {
  ...
  test('Email service validation', async ({ page }) => {
    ...
    try {
      const testResponse = await page.request.post(`${API_BASE_URL}/api/auth/request-password-reset`, {
        data: { email: 'diagnostic-test@example.com' },
        headers: { 'Content-Type': 'application/json' }
      });
...
test.describe('Password Reset Diagnostic - Summary Report', () => {
  ...
  test('Generate comprehensive diagnostic report', async ({ page }) => {
    ...
    // Create summary page
    await page.goto('data:text/html,<h1>Password Reset Diagnostic Complete</h1><p>Check console for detailed findings</p>');
