...
class CompetitorAnalyzer {
  ...
  async analyzeCompetitor(config: any, testUrl: any): Promise<CompetitorTestResult> {
    ...
    try {
      ...
      if (output) {
        ...
        console.log(
          `✅ Success: ${result.fileSize} chars, ${result.pagesFound} pages, quality: ${result.contentStructure.qualityScore}`
        );
      } else {
  ...
  generateComparisonReport(): string {
    ...
    report.push(
      `**Test URLs**: ${TEST_URLS.map((u) => (typeof u === 'string' ? u : u.url)).join(', ')}\n`
    );
    ...
    const blocked = this.results.filter((r) =>
      ['blocked', 'rate_limited', 'captcha'].includes(r.status)
    );
    ...
    if (successful.length > 0) {
      ...
      const avgFileSize =
        successful.reduce((sum, r) => sum + (r.fileSize || 0), 0) / successful.length;
      const avgPagesFound =
        successful.reduce((sum, r) => sum + (r.pagesFound || 0), 0) / successful.length;
      const avgQualityScore =
        successful.reduce((sum, r) => sum + (r.contentStructure?.qualityScore || 0), 0) /
        successful.length;
    ...
    if (successful.length > 1) {
      ...
      for (const result of successful) {
        ...
        report.push(
          `| ${result.competitor} | ${result.processingTime}ms (${performance.timeRating}) | ${result.fileSize} chars (${performance.sizeRating}) | ${result.pagesFound} (${performance.pageRating}) | ${result.contentStructure?.qualityScore || 0}/100 |`
        );
      }
      report.push('');
    }
    ...
    for (const result of this.results) {
      ...
      if (result.processingTime) {
        ...
        report.push(
          `- **Processing Time**: ${result.processingTime}ms (${performance.timeRating})`
        );
      }
      ...
      if (result.contentStructure) {
        ...
        report.push(
          `  - Has Substantial Content: ${result.contentStructure.hasContent ? '✅' : '❌'}`
        );
    ...
    if (successful.length > 0) {
      ...
      const fastest = successful.reduce((min, r) =>
        (r.processingTime || Infinity) < (min.processingTime || Infinity) ? r : min
      );
      const mostPages = successful.reduce((max, r) =>
        (r.pagesFound || 0) > (max.pagesFound || 0) ? r : max
      );
      const highestQuality = successful.reduce((max, r) =>
        (r.contentStructure?.qualityScore || 0) > (max.contentStructure?.qualityScore || 0)
          ? r
          : max
      );
      ...
      report.push(
        `- **Most Pages Discovered**: ${mostPages.competitor} (${mostPages.pagesFound} pages)`
      );
      report.push(
        `- **Highest Quality Output**: ${highestQuality.competitor} (${highestQuality.contentStructure?.qualityScore}/100)`
      );

      report.push(`\n**Key Differentiators**:`);
      report.push(
        `- Processing speed varies significantly (${Math.min(...successful.map((r) => r.processingTime || 0))}ms - ${Math.max(...successful.map((r) => r.processingTime || 0))}ms)`
      );
      report.push(
        `- Page discovery capabilities range from ${Math.min(...successful.map((r) => r.pagesFound || 0))} to ${Math.max(...successful.map((r) => r.pagesFound || 0))} pages`
      );
      report.push(
        `- Content quality scores range from ${Math.min(...successful.map((r) => r.contentStructure?.qualityScore || 0))} to ${Math.max(...successful.map((r) => r.contentStructure?.qualityScore || 0))}/100`
      );
    }
    ...
    test(`${config.name} - LLMs.txt generator analysis`, async ({ page }) => {
      ...
      for (const testUrl of TEST_URLS) {
        try {
          ...
          console.log(
            `📊 ${config.name} result for ${typeof testUrl === 'string' ? testUrl : testUrl.url}: ${result.status}`
          );
    ...
    try {
      ...
      const jsonData = {
        ...
        summary: {
          ...
          blocked: analyzer
            .getResults()
            .filter((r) => ['blocked', 'rate_limited', 'captcha'].includes(r.status)).length,
        },
      };
      ...
      if (successful.length > 0) {
        ...
        successful.forEach((r) => {
          console.log(
            `  • ${r.competitor}: ${r.fileSize} chars, ${r.pagesFound} pages, ${r.contentStructure?.qualityScore}/100 quality (${r.processingTime}ms)`
          );
        });
      } else {
        console.log(
          `❌ No competitors successfully analyzed. Check the detailed report for issues.`
        );
      }
