...
test.describe('Conversion Validation Tests', () => {
  ...
  test.describe('Test Suite 1: New User Signup Flow', () => {
    test('should verify Coffee tier is pre-selected by default', async ({ page }) => {
      ...
      await test.step('Verify Coffee tier is default selection', async () => {
        ...
        const coffeeContainer = page
          .locator('[data-testid="coffee-tier"], .border-orange-400, .bg-orange-50')
          .first();
  ...
  test.describe('Test Suite 3: Conversion Metrics', () => {
    ...
    test('should validate conversion funnel metrics', async ({ page }) => {
      ...
      console.log(
        `- Tier Selections: ${metrics.tierSelections} (${(tierSelectionRate * 100).toFixed(1)}%)`
      );
      console.log(
        `- Signup Attempts: ${metrics.signupAttempts} (${(signupAttemptRate * 100).toFixed(1)}%)`
      );
      console.log(
        `- Signup Completions: ${metrics.signupCompletions} (${(signupCompletionRate * 100).toFixed(1)}%)`
      );
