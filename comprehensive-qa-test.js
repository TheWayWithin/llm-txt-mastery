...
async function testComprehensively() {
  ...
  try {
    ...
    for (let i = 0; i < navLinks.length; i++) {
      ...
      if (href && !href.startsWith('#') && !href.startsWith('http')) {
        ...
        try {
          ...
          if (currentUrl.includes('404') || await page.locator('text=/404|not found/i').count() > 0) {
          ...
          await logIssue('MEDIUM', 'Navigation', `Link navigation failed: ${text}`, { href, error: error.message });
        }
      }
    }
      ...
      if (href && !href.startsWith('mailto:') && !href.startsWith('#')) {
        ...
        if (href.startsWith('http')) {
          ...
          try {
            ...
            if (page.url().includes('404') || await page.locator('text=/404|not found/i').count() > 0) {
            ...
            await logIssue('LOW', 'Navigation', `Footer link issue: ${text}`, { href, error: error.message });
          }
        }
      }
    }
    ...
    // Scroll to URL input
    await page.evaluate(() => {
      document.querySelector('input[placeholder*="example.com"]')?.scrollIntoView({ behavior: 'smooth' });
    });
    ...
    if (await urlInput.count() === 0) {
      ...
      if (await analyzeBtn.count() === 0) {
        ...
        if (await emailInput.count() > 0) {
          ...
          // Continue
          const continueBtn = page.locator('button:has-text("Continue"), button:has-text("Quick Start"), button:has-text("Get Started")').first();
          if (await continueBtn.count() > 0) {
            await continueBtn.click();
            console.log('✅ Clicked continue');
            
            // Wait for analysis
            const analysisStarted = await page.waitForSelector('text=/Analyzing|Processing|Discovering/i', { timeout: 10000 }).catch(() => null);
            if (!analysisStarted) {
              ...
              // Wait for completion
              const analysisComplete = await page.waitForSelector('text=/found|complete|Review/i', { timeout: 60000 }).catch(() => null);
              if (analysisComplete) {
                ...
                if (await page.locator('text=/Review.*Select|Select.*Pages/i').count() > 0) {
                  ...
                  if (await generateBtn.count() > 0) {
                    ...
                    if (await downloadBtn.count() > 0) {
                      ...
                      await logIssue('MEDIUM', 'Functionality', 'Download button not found after generation');
                    }
                  }
                }
              } else {
    ...
    // Continue
    const coffeeContBtn = page.locator('button:has-text("Continue"), button:has-text("Quick Start"), button:has-text("Get Started")').first();
    ...
    const mobileMenu = page.locator('button[aria-label*="menu"], button:has-text("Menu"), [class*="burger"], [class*="mobile-menu"]').first();
    if (await mobileMenu.count() > 0) {
      ...
      const menuOpen = await page.locator('nav:visible, [class*="menu-open"], [class*="mobile-nav"]').count() > 0;
      if (menuOpen) {
    ...
    if (contentWidth > viewportWidth + 20) { // Allow 20px margin
      await logIssue('MEDIUM', 'Responsive', `Content overflows mobile viewport (${contentWidth}px vs ${viewportWidth}px)`);
    } else {
    ...
    await page.fill('input[placeholder*="example.com"]', 'https://this-domain-definitely-does-not-exist-12345.com');
    await page.click('button:has-text("Analyze Website")');
