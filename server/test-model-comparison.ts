...
Respond in JSON format: {"title": "", "description": "", "qualityScore": 0, "category": "", "relevance": 0}`;
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert at analyzing web content for LLM training data quality. Respond only with valid JSON.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    if (modelName === 'gpt-4o') {
      ...
      inputCost = (usage.prompt_tokens / 1_000_000) * 2.5;
      outputCost = (usage.completion_tokens / 1_000_000) * 10.0;
    } else if (modelName === 'gpt-4o-mini') {
      // GPT-4o-mini pricing: $0.15/1M input, $0.60/1M output
      inputCost = (usage.prompt_tokens / 1_000_000) * 0.15;
      outputCost = (usage.completion_tokens / 1_000_000) * 0.6;
    } else if (modelName === 'gpt-3.5-turbo') {
      // GPT-3.5-turbo pricing: $0.50/1M input, $1.50/1M output
      inputCost = (usage.prompt_tokens / 1_000_000) * 0.5;
      outputCost = (usage.completion_tokens / 1_000_000) * 1.5;
    }
...
async function runComparison() {
  ...
  for (const result of results) {
    ...
    console.log(
      `Cost: $${result.cost.total.toFixed(6)} ($${result.cost.input.toFixed(6)} input + $${result.cost.output.toFixed(6)} output)`
    );
