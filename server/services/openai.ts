...
function generateHTMLAnalysis(url: string, htmlContent: string): ContentAnalysisResult {
  ...
  const isNavigationPage =
    textContent.toLowerCase().includes('from here you can') ||
...
Focus on technical accuracy, information density, and AI utility.`;
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert content analyst specializing in AI/LLM accessibility. Respond only with valid JSON.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    return {
      ...
      qualityScore: Math.max(
        1,
        Math.min(10, parseInt(aiResult.qualityScore) || htmlResult.qualityScore)
      ),
...
function extractStructuredContent($: any, url: string): string | null {
  ...
  if (urlLower.includes('/api')) {
    const endpoints = $('code, .endpoint, .method')
      .map((_, el) => $(el).text().trim())
      .get()
      .slice(0, 3);
...
function isNavigationText(text: string): boolean {
  ...
  return (
    navPatterns.some((pattern) => pattern.test(text)) ||
...
function applyCategoryQualityAdjustments(
  ...
  switch (category) {
    case 'API Reference':
      ...
      if (contentLower.includes('parameter') && contentLower.includes('response'))
        adjustedScore += 0.5;
      break;
