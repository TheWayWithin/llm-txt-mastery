...
export class ConnectionPoolManager {
  ...
  private cleanupIdleAgents(): void {
    ...
    if (hostnamesToCleanup.length > 0) {
      console.log(
        `Cleaned up ${hostnamesToCleanup.length} idle agents. Active agents: ${this.agents.size}`
      );
    }
  }
  ...
  private evictOldestAgent(): void {
    ...
    if (oldestHostname) {
      ...
      if (agent) {
        ...
        console.log(
          `Evicted oldest agent for ${oldestHostname} (was ${Math.round((Date.now() - oldestTime) / 1000)}s old)`
        );
      }
    }
  }
