...
export async function incrementSimpleUsage(
  email: string,
  tier: string = 'starter'
): Promise<number> {
  if (!email) {
  ...
  console.log(
    `🔍 [SIMPLE-TRACKER] INCREMENT called for: "${email}" -> normalized: "${normalizedEmail}" on ${today} with tier: ${tier}`
  );
...
export async function getSimpleUsage(email: string): Promise<{ count: number; tier: string }> {
  ...
  console.log(
    `🔍 [SIMPLE-TRACKER] GET called for: "${email}" -> normalized: "${normalizedEmail}" on ${today}`
  );
