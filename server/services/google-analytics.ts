  ...
  try {
    analyticsClient = new BetaAnalyticsDataClient({
      ...
      credentials: process.env.GA4_SERVICE_ACCOUNT_KEY_JSON ? 
        JSON.parse(process.env.GA4_SERVICE_ACCOUNT_KEY_JSON) : undefined,
    });
    ...
    console.warn('⚠️ Google Analytics initialization failed (running without Analytics):', error.message);
...
export async function generateContentInsights(
  ...
  try {
    ...
    const performanceData: ContentPerformance[] = pageAnalytics.map(page => {
      ...
      const traffic_score = maxSessions > 0 ? 
        Math.min(10, Math.max(1, (page.sessions / maxSessions) * 10)) : 1;
      ...
      const duration_boost = maxDuration > 0 ? 
        (page.avg_session_duration / maxDuration) * 3 : 0;
      ...
      const conversion_value = page.conversion_rate ? 
        Math.min(10, Math.max(1, page.conversion_rate * 100)) : 5;
    ...
    const trafficSources = pageAnalytics.reduce((acc, page) => {
    ...
    const deviceBreakdown = pageAnalytics.reduce((acc, page) => {
...
export async function getHighPriorityUrls(
  ...
  try {
    ...
    const priorityUrls = insights.top_performing_pages
      .slice(0, limit)
      .map(page => {
        let reason = '';
