...
function arePagesNearDuplicates(page1: DiscoveredPage, page2: DiscoveredPage): boolean {
  ...
  // For both exact matches and high similarity, check URL paths first.
  // CSR/SPA sites serve the same index.html for all routes, so every page
  // has the identical title. Without the path check, all pages collapse into one.
  if (title1 === title2 || calculateStringSimilarity(title1, title2) > 0.8) {
    try {
      const path1 = new URL(page1.url).pathname.replace(/\/$/, '').toLowerCase();
      const path2 = new URL(page2.url).pathname.replace(/\/$/, '').toLowerCase();
      if (path1 !== path2) {
        // Different paths -- keep both (likely a CSR site with shared title)
        return false;
      }
...
function areUrlPathsSimilar(url1: string, url2: string): boolean {
  try {
    ...
    for (const variation of indexVariations) {
      if (normPath1 === normPath2 + variation || normPath2 === normPath1 + variation) {
        return true;
      }
    }
...
export function isLikelyBrokenUrl(url: string): boolean {
  try {
    ...
    const suspiciousPatterns = [/localhost/, /127\.0\.0\.1/, /\.\./, /\s/, /[<>]/];
...
export function removeAffiliateAndLowValueLinks(pages: DiscoveredPage[]): DiscoveredPage[] {
  ...
  const result = pages.filter((page) => {
    ...
    if (page.description.toLowerCase().includes('read more') && page.description.length < 100) {
      return false;
    }
