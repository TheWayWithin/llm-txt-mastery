...
class APIKeyStorage {
  ...
  checkRotationStatus(keyId: string): {
    ...
    const now = new Date();
    const daysUntilDue = Math.ceil((keyInfo.rotation_due.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
  ...
  getKeysNeedingAttention(): Array<{
    ...
    return needingAttention.sort((a, b) => 
      a.rotation_status.days_until_due - b.rotation_status.days_until_due
    );
  }
  ...
  completeRotation(oldKeyId: string, newKey: string): string {
    ...
    console.log(`[Key Manager] Completed rotation for ${oldKeyInfo.service}: ${oldKeyId} -> ${newKeyId}`);
    return newKeyId;
  }
  ...
  getKeyStats(): {
    ...
    for (const [keyId, keyInfo] of this.keys) {
      ...
      if (!stats.by_service[keyInfo.service].last_rotation || 
...
export function initializeKeyTracking(): void {
  ...
  if (process.env.GA4_SERVICE_ACCOUNT_KEY_JSON || process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    keyStorage.registerKey('google_analytics', 
