...
export function validateSemanticConfig(): { isValid: boolean; errors: string[] } {
  ...
  // OpenAI validation
  if (!['text-embedding-3-small', 'text-embedding-3-large', 'text-embedding-ada-002'].includes(semanticConfig.openai.model)) {
  ...
  if (semanticConfig.openai.dimensions !== 1536 && semanticConfig.openai.dimensions !== 3072) {
    errors.push('EMBEDDING_DIMENSIONS must be 1536 (text-embedding-3-small) or 3072 (text-embedding-3-large)');
  }
  ...
  if (semanticConfig.clustering.coherenceThreshold < 0 || semanticConfig.clustering.coherenceThreshold > 1) {
    errors.push('CLUSTERING_COHERENCE_THRESHOLD must be between 0 and 1');
  }
  
  // Thresholds validation
  if (semanticConfig.analysis.similarityThreshold < 0 || semanticConfig.analysis.similarityThreshold > 1) {
    errors.push('SEMANTIC_SIMILARITY_THRESHOLD must be between 0 and 1');
  }
  
  if (semanticConfig.analysis.uniquenessThreshold < 0 || semanticConfig.analysis.uniquenessThreshold > 1) {
    errors.push('UNIQUENESS_SCORE_THRESHOLD must be between 0 and 1');
  }
  
  if (semanticConfig.analysis.tagConfidenceThreshold < 0 || semanticConfig.analysis.tagConfidenceThreshold > 1) {
