...
`;
  ...
  // Add metadata section with generation info (preserves analytics value)
  let metadataSection = `## Generated Metadata\n\n`;
  metadataSection += `Generated by [LLM.txt Mastery](https://llmtxt.com) on ${createdDate}\n\n`;
  metadataSection += `### Analysis Summary\n\n`;
  metadataSection += `- Pages Found: ${totalFound} (discovered in sitemap and crawling)\n`;
  metadataSection += `- Pages Analyzed: ${analyzed} (successfully fetched and scored)\n`;
  metadataSection += `- Pages Included: ${enhancedPages.length} (selected for this file)\n`;
  metadataSection += `- Pages Excluded: ${excluded} (filtered during review)\n\n`;

  metadataSection += `### Content Quality Metrics\n\n`;
  metadataSection += `- Average Quality Score: ${avgQuality}/10 (${avgQuality >= 7 ? 'High' : avgQuality >= 5 ? 'Medium' : 'Improving'} overall quality)\n`;
  metadataSection += `- Estimated Total Content: ~${estimatedWordCount.toLocaleString()} words across all pages\n`;
  metadataSection += `- Site Architecture: ${siteStructure.clusterCount} content clusters, max depth ${siteStructure.maxDepth} levels\n`;
  metadataSection += `- Content Distribution: ${siteStructure.largestClusterSize} pages in largest cluster\n`;
  metadataSection += `- Common Content Types: ${commonTags.join(', ') || 'Mixed content'}\n`;
  metadataSection += `- Technical Features: ${techFeatures.length > 0 ? techFeatures.join(', ') : 'Standard web content'}\n\n`;

  // Add excluded pages section if any were filtered
  let excludedSection = '';
  if (excludedPages.length > 0) {
    excludedSection += `### Excluded Pages\n\n`;
    excludedSection += `The following ${excluded} pages were excluded due to lower quality scores, content duplication, or limited relevance:\n\n`;
    excludedSection += excludedPages
  ...
  // Add footer with links to analysis tools
  let footer = `### Additional Resources\n\n`;
  footer += `- [Review Full Analysis](https://llmtxt.com/analysis/${analysisMetadata?.analysisId || 'view'}) - View quality scores and make changes\n`;
  footer += `- [Learn About llms.txt Format](https://llmstxt.org/) - Official specification\n`;
  footer += `- [Contact Support](https://llmtxt.com/contact) - Questions about this analysis\n`;

  return header + content + metadataSection + excludedSection + footer;
}
