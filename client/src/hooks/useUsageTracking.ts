...
class ClientUsageTracker {
  ...
  static incrementUsage(email: string): UsageData {
    ...
    console.log(
      `📊 [CLIENT] Usage incremented for ${email}: ${data[email].currentUsage}/${data[email].dailyAnalyses}`
    );
    return data[email];
  }
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const simpleResponse = await apiRequest(
          'GET',
          `/api/simple-usage/${encodeURIComponent(email)}`
        );
        if (simpleResponse.ok) {
          ...
          if (simpleData.tier === 'solo' && simpleData.creditsRemaining === undefined) {
            ...
            console.log(
              `📊 [SIMPLE] Usage fetched for ${email}: ${usage.currentUsage}/${usage.dailyAnalyses}${usage.creditsRemaining !== undefined ? ` (${usage.creditsRemaining} credits)` : ''} - tier: ${usage.tier}`
            );
            return usage;
          }
        }
      ...
      try {
        ...
        console.log(
          `📊 [SERVER] Usage fetched for ${email}: ${usage.currentUsage}/${usage.dailyAnalyses}${usage.creditsRemaining !== undefined ? ` (${usage.creditsRemaining} credits)` : ''}`
        );
        return usage;
      } catch (error) {
  ...
  const trackUsageMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        const data = await response.json();
        console.log(
          `✅ [SIMPLE-SERVER] Usage tracked for ${email}: count=${data.count}, tier=${data.tier}`
        );
