...
export function useABTest(experimentName: string): UseABTestReturn {
  ...
  const fetchAssignment = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to get experiment assignment';
  ...
  const trackEvent = useCallback(
    async (eventType: string, eventValue?: number, eventProperties?: Record<string, any>) => {
...
export function useMultipleABTests(experimentNames: string[]): UseMultipleABTestsReturn {
  ...
  const fetchAssignments = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to get experiment assignments';
      setError(errorMessage);
...
export function ABTestGate({
  ...
  if (loading) {
    return (
      <>{loadingComponent || <div className="animate-pulse bg-mist h-4 w-16 rounded"></div>}</>
    );
  }
