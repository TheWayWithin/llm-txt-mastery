...
describe('EmailCapture Component - Characterization Tests', () => {
  ...
  describe('Header Content', () => {
    ...
    it('displays fallback message when no website URL provided', () => {
      ...
      expect(screen.getByText(/Select your tier, enter your email, and we'll help you create/)).toBeInTheDocument()
    })
  })
  ...
  describe('Tier Selection Behavior', () => {
    ...
    it('tracks analytics events when tier is selected', async () => {
      ...
      await waitFor(() => {
        expect(analytics.trackEvent).toHaveBeenCalledWith('tier_selected', expect.objectContaining({
  ...
  describe('Authentication Navigation', () => {
    ...
    it('tracks analytics for authentication clicks', async () => {
      ...
      await user.click(screen.getByText('Sign In'))
      expect(analytics.trackEvent).toHaveBeenCalledWith('login_click', expect.objectContaining({
      ...
      await user.click(screen.getByText('Sign Up'))
      expect(analytics.trackEvent).toHaveBeenCalledWith('signup_click', expect.objectContaining({
        tier_selected: 'coffee',
        website_url: 'https://example.com',
        event_category: 'auth'
      }))
  ...
  describe('Guarantee Section', () => {
    it('displays comprehensive guarantee section', () => {
      ...
      expect(screen.getByText('🛡️ ZERO RISK GUARANTEE - We Remove ALL Your Fears')).toBeInTheDocument()
    ...
    it('shows trust indicators', () => {
      ...
      expect(screen.getByText(/Secure & Private.*No Spam Ever.*Built by Expert Solopreneur/)).toBeInTheDocument()
    })
  })
