...
export default function ContentAnalysis({ 
  ...
  function getDetailedAnalysisError(error: any): string {
    ...
    return error.message || "Analysis failed due to an unexpected error. Please try again or contact support.";
  }
  ...
  if (lastError || (error && !analysisData)) {
    const displayError = lastError || (error instanceof Error ? error.message : "Failed to analyze website");
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {canRetry && (
                ...
                >
                  ...
                  {startAnalysisMutation.isPending ? "Retrying..." : `Try Again ${retryCount > 0 ? `(${retryCount + 1}/3)` : ""}`}
                </Button>
              )}
            ...
            {retryCount >= 3 && (
              <div className="text-center pt-4 border-t">
                ...
                <Button
                  variant="outline"
                  onClick={() => window.open('mailto:support@llmtxtmastery.com?subject=Analysis Error&body=Error: ' + encodeURIComponent(displayError), '_blank')}
                >
                  Contact Support
                </Button>
              </div>
