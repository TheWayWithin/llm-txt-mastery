...
module.exports = (results) => {
  ...
  const meetsCoverage = summary.coverage ? 
    summary.coverage.statements >= (qualityGate.minCoverage * 100) : false;
  const meetsDuration = summary.duration <= qualityGate.maxDuration;
  
  if (!meetsPassRate) {
    console.warn(`⚠️ Pass rate ${(passRate * 100).toFixed(1)}% below threshold ${(qualityGate.minPassRate * 100)}%`);
  }
  
  if (summary.coverage && !meetsCoverage) {
    console.warn(`⚠️ Coverage ${summary.coverage.statements}% below threshold ${qualityGate.minCoverage * 100}%`);
  }
  
  if (!meetsDuration) {
    console.warn(`⚠️ Duration ${(summary.duration / 1000).toFixed(2)}s above threshold ${qualityGate.maxDuration / 1000}s`);
  }
