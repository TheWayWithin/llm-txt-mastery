...
export class DynamicBatchCalculator {
  ...
  async calculateOptimalBatch(entries: SitemapEntry[], tier: UserTier): Promise<BatchConfig> {
    ...
    const avgResponseTime =
      history.length > 0 ? history.reduce((a, b) => a + b, 0) / history.length : 2000; // Default 2 seconds
...
async function performPageAnalysisWithCache(
  ...
  for (let i = 0; i < pagesToAnalyze.length; i += BATCH_SIZE * CONCURRENT_BATCHES) {
    ...
    for (let j = 0; j < CONCURRENT_BATCHES && i + j * BATCH_SIZE < pagesToAnalyze.length; j++) {
      ...
      const startTime = Date.now();
      batchPromises.push(
        processBatchWithCache(batch, userEmail, tier, tierLimits.aiPagesLimit, metrics).then(
          (result) => {
