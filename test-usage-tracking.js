...
async function testUsageTracking() {
  ...
  try {
    ...
    const usageResponse1 = await fetch(
      `https://llm-txt-mastery-production.up.railway.app/api/usage/${testEmail}`
    );
    ...
    const analyzeResponse = await fetch(
      'https://llm-txt-mastery-production.up.railway.app/api/analyze',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    const statusResponse = await fetch(
      `https://llm-txt-mastery-production.up.railway.app/api/analysis/${analyzeData.analysisId}`
    );
    ...
    const usageResponse2 = await fetch(
      `https://llm-txt-mastery-production.up.railway.app/api/usage/${testEmail}`
    );
    ...
    if (usage2.usage.analysesToday > usage1.usage.analysesToday) {
      console.log(
        `   ✅ Usage tracking WORKING! Count increased from ${usage1.usage.analysesToday} to ${usage2.usage.analysesToday}`
      );
    } else {
    ...
    const usageResponse3 = await fetch(
      `https://llm-txt-mastery-production.up.railway.app/api/usage/${testEmail}`
    );
