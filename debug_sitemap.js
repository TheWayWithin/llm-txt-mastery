...
async function testFullSitemapFlow() {
  try {
    ...
    if (sitemapResult.entries.length > 0) {
      ...
      console.log(
        `Starter tier filtering: ${sitemapResult.entries.length} → ${starterFiltered.length} pages`
      );

      // Test coffee tier (paid)
      const coffeeFiltered = filterRelevantPages(sitemapResult.entries, 'coffee');
      console.log(
        `Coffee tier filtering: ${sitemapResult.entries.length} → ${coffeeFiltered.length} pages`
      );
      ...
      if (starterFiltered.length === 0) {
        ...
        const excludePatterns = [
          {
            pattern: /\.(jpg|jpeg|png|gif|pdf|zip|xml|json|css|js|woff|woff2|ttf|eot|ico|svg)$/i,
