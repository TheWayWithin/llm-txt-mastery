...
const testGDPRCompliance = async () => {
  ...
  try {
    ...
    console.log(
      `  ✅ /privacy route: ${response1.status === 200 ? 'PASS' : 'FAIL'} (Status: ${response1.status})`
    );
    console.log(
      `  ✅ /privacy-policy route: ${response2.status === 200 ? 'PASS' : 'FAIL'} (Status: ${response2.status})`
    );
  } catch (error) {
  ...
  Object.entries(envVars).forEach(([key, value]) => {
    ...
    console.log(
      `  ${isConfigured ? '✅' : '⚠️ '} ${key}: ${isConfigured ? 'CONFIGURED' : 'NOT CONFIGURED'}`
    );
  });
