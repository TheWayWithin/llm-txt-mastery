...
async function debugFormSubmission() {
  ...
  try {
    ...
    const formHandler = await page.evaluate(() => {
      ...
      if (form) {
        return {
          ...
          hasEventListeners: form.getEventListeners ? !!form.getEventListeners('submit') : 'unknown',
          action: form.action,
          method: form.method
        };
      }
    ...
    const errorMessages = await page.locator('[class*="error"], [class*="destructive"], .text-red-500, .text-red-700').allTextContents();
