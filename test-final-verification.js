...
async function runFinalVerification() {
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      ...
      if (errorText.includes('useRoutes') || errorText.includes('Router') || errorText.includes('basename')) {
  ...
  try {
    ...
    console.log(results.routerMigration ? '✅ PASS: No React Router errors' : '❌ FAIL: Router errors detected');
    ...
    console.log(results.emailCaptureFirst ? '✅ PASS: Email form visible on landing' : '❌ FAIL: Email form not visible');
    ...
    console.log(results.tierSelectionWorks ? 
    ...
    try {
      ...
      const submitButton = page.locator('button[type="submit"]').filter({ hasText: /start|submit|continue/i });
      ...
      if (hasSubmitButton) {
        ...
        console.log(results.freeFlowWorks ? 
    ...
    try {
      ...
      // Submit form
      const submitButton2 = page.locator('button[type="submit"]').filter({ hasText: /start|submit|continue/i });
      ...
      if (hasSubmitButton2) {
        ...
        const hasStripeElements = await page.evaluate(() => {
          return document.querySelector('*[class*="stripe"], *[id*="stripe"]') !== null ||
                 document.body.textContent?.includes('Stripe') ||
                 document.body.textContent?.includes('checkout');
        });
        
        results.coffeeFlowWorks = currentUrl.includes('stripe') || 
                                  currentUrl.includes('checkout') || 
                                  hasStripeElements ||
                                  currentUrl !== 'http://localhost:8080/';
        
        console.log(results.coffeeFlowWorks ? 
    ...
    console.log(results.mobileResponsive ? 
    ...
    console.log(`🚨 Console Errors: ${consoleErrors.length} (${routerErrors.length} router-related)`);
    
    // Production readiness verdict
    const criticalTests = [results.routerMigration, results.emailCaptureFirst, results.tierSelectionWorks];
