...
async function testAllFixes() {
  ...
  try {
    ...
    const quickStartButton = page
      .getByRole('button', { name: /quick start|continue without/i })
      .first();
    ...
    console.log(
      `   ${results.usageTracking.first ? '✅ PASS' : '❌ FAIL'}: Usage tracking for first analysis`
    );
    ...
    console.log(
      `   ${results.usageTracking.second ? '✅ PASS' : '❌ FAIL'}: Usage tracking for second analysis`
    );
    ...
    console.log(
      `   ${results.usageTracking.third ? '✅ PASS' : '❌ FAIL'}: Usage tracking for third analysis`
    );
    ...
    if (!limitBlocked) {
      ...
      const analysisStarted = await page
        .locator('text=/analyzing|discovering/i')
    ...
    if (await analyzeAnotherBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      console.log(
        `   ${results.scrollPosition ? '✅ PASS' : '❌ FAIL'}: Scrolled to URL input field`
      );
    } else {
    ...
    // Get page count
    const pageCountElement = page
      .locator('text=/\\d+\\s*pages?\\s*(discovered|analyzed)/i')
      .first();
    if (await pageCountElement.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      console.log(
        `   ${results.pageCount ? '✅ PASS' : '❌ FAIL'}: Expected 19-20 pages, got ${pageCount}`
      );
    } else {
    ...
    if (await banner.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const dismissButton = page
        .locator('button[aria-label*="Dismiss"], button[title*="Dismiss"], .absolute button')
        .first();
      if (await dismissButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (bannerGone) {
          ...
          console.log(
            `   ${results.bannerDismiss ? '✅ PASS' : '❌ FAIL'}: Banner stays dismissed after refresh`
          );
        } else {
