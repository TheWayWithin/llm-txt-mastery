...
| LTM-ISS-9 | llmtxtmastery.com is a client-rendered SPA: non-JS crawlers (and AImpactScanner: 36/100 F, heading hierarchy 0, content depth 5) see an empty shell — the llms.txt product is itself near-invisible to AI. Add prerendering/SSG for the marketing pages | Open | high | — | pending |
...
| LTM-ISS-2 | TS baseline burn-down: 261 → 12 (2026-07-25, commits 4344073..7ca34df). All 12 residual errors are the broken /api/v1 processAnalysis pipeline (LTM-ISS-5) — a design repair (which identity to bill/track) not a type fix; gate blocks all NEW errors (proven) and closes when LTM-ISS-5 lands | Open | low | — | pending |
