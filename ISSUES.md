...
| LTM-ISS-10 | No structured data on llmtxtmastery.com: zero ld+json (add Organization/WebSite/SoftwareApplication + FAQPage if honest), no date metadata, llms.txt undated | Fixed — pending review (branch ltm-iss10-schema-dates, 2026-07-25): Organization+WebSite on all 9 marketing routes, SoftwareApplication with the 4 real monthly prices on / and /pricing, FAQPage on / built from the SAME constant as the visible 7-item FAQ; visible "Content updated" + time[datetime] on every marketing page (legal pages keep their own policy dates); llms.txt + llms-full.txt dated 2026-07-25. No-JS proof: / has 4 valid ld+json blocks, /pricing 3, all parse; seo-audit.sh 0 fails (ld+json PASS, date metadata PASS); vitest 142 passed. Merge+deploy, rescan AImpactScanner (vs 73), update PRJ-25, close via repo-done.py | medium | — | pending |
