...
|----|-------|--------|----------|--------|---------|
| LTM-ISS-12 | Canonical tag mismatch: head declares https://www.llmtxtmastery.com while the site serves non-www — scanner flags cross-domain canonical (40/100) and www/non-www inconsistency; fix canonical (and per-page canonicals) as part of the LTM-ISS-10 meta work | Fixed — pending review (branch ltm-iss12-canonical, 2026-07-26): useSEO now stamps a self-referential non-www canonical + og:url per route from the shared SITE_URL constant (captured into the prerendered HTML); shell defaults fixed to https://llmtxtmastery.com/; visible www mentions in privacy/cookies and test-tooling URLs swept. Verified: all 9 routes canonical==own non-www URL with og:url matching, zero www in served pages/sitemap/JSON-LD, seo-audit canonical PASS, vitest 142 passed. www already 301s to non-www with path preserved (Netlify primary domain) — no redirect work needed. Merge+deploy, curl live, rescan vs 81, close via repo-done.py | low | — | pending |
| LTM-ISS-11 | Perf + policy gaps on llmtxtmastery.com: LCP 6.5s (LH perf .70), robots.txt has no explicit AI-crawler policy (GPTBot/ClaudeBot/PerplexityBot/Google-Extended unnamed) | Fixed — pending review (branch ltm-iss11-perf-robots, 2026-07-26): robots.txt now names GPTBot, OAI-SearchBot, ClaudeBot, Claude-Web, PerplexityBot, Google-Extended with explicit Allow (seo-audit: all robots checks PASS, 0 fails). Perf half: target already MET by the LTM-ISS-9 prerender — warm local production build measures LH 0.97 / LCP 2.1s observed (devtools throttling); the old 6.5s/.70 was the pre-prerender production page. Font self-hosting was prototyped, measured as no-win (and it trips Lighthouse's simulated model into a false 9.7s LCP that PageSpeed would display), and deliberately dropped — evidence in the branch commit. vitest 142 passed | medium | — | pending |
