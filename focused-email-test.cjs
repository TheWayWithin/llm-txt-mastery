...
async function testEmailFlowWithRadixUI() {
  ...
  page.on('console', (msg) => {
    ...
    if (
      text.includes('EMAIL_CAPTURED') ||
      text.includes('captureEmail') ||
      text.includes('onEmailCaptured')
    ) {
  ...
  try {
    ...
    for (const tier of tiers) {
      ...
      const radioGroupItem = page.locator(
        `[role="radio"][data-state][aria-labelledby], [role="radio"][value="${tier.name}"]`
      );
      ...
      if ((await submitButton.count()) === 0) {
        ...
        if (tier.name === 'coffee') {
          submitButton = page.locator(
            'button:has-text("Continue to Payment"), button:has-text("Pay")'
          );
        } else {
