...
class PerformanceTestingHarness {
  ...
  private async simulateEndToEndSemanticAnalysis(
    website: TestWebsite
  ): Promise<Record<string, number>> {
    const pages = website.pages;
    ...
    for (const result of this.results) {
      ...
      if (result.testName.startsWith('clustering_')) {
        ...
        if (pageCount) {
          ...
          if (threshold && result.duration > threshold) {
            console.log(
              `⚠️  ${result.testName}: Duration ${result.duration.toFixed(2)}ms > ${threshold}ms`
            );
            violations++;
          }
        }
      ...
      if (result.metrics) {
        if (
          ...
          result.metrics.coherenceScore < this.thresholds.clustering.minCoherenceScore
        ) {
          console.log(
            `⚠️  ${result.testName}: Coherence score ${result.metrics.coherenceScore.toFixed(2)} < ${this.thresholds.clustering.minCoherenceScore}`
          );
          violations++;
        }
        ...
        if (
          ...
          result.metrics.cacheHitRate < this.thresholds.embeddings.cacheHitRateMin
        ) {
          console.log(
            `⚠️  ${result.testName}: Cache hit rate ${(result.metrics.cacheHitRate * 100).toFixed(1)}% < ${(this.thresholds.embeddings.cacheHitRateMin * 100).toFixed(1)}%`
          );
          violations++;
        }
      }
