...
try {
  ...
  try {
    ...
    if (connectionTest.success) {
      ...
      if (connectionTest.embeddings) {
        console.log(`   - Embeddings: ${connectionTest.embeddings.response_time_ms}ms, ${connectionTest.embeddings.tokens_used} tokens, $${connectionTest.embeddings.cost_usd.toFixed(6)}`);
        console.log(`   - Model: ${connectionTest.embeddings.model}, Dimensions: ${connectionTest.embeddings.dimensions}`);
      }
      if (connectionTest.chat) {
        console.log(`   - Chat: ${connectionTest.chat.response_time_ms}ms, $${connectionTest.chat.cost_usd.toFixed(6)}`);
      }
    } else {
  ...
  try {
    const testText = "This is a comprehensive guide to building scalable web applications with React and Node.js";
  ...
  try {
    if (window.batchResults && window.batchResults.length >= 2) {
      ...
      console.log(`   - JavaScript ↔ React: ${similarity1.toFixed(4)} (should be high - both web dev)`);
      ...
      similar.forEach((match, i) => {
        console.log(`     ${i + 1}. "${match.metadata.text.substring(0, 40)}..." (${match.similarity.toFixed(4)})`);
      });
    }
  } catch (error) {
  ...
  try {
    ...
    sampleTexts.forEach((text, i) => {
      ...
      console.log(`   - Text ${i + 1} (${text.length} chars): ${estimate.tokens} tokens, $${estimate.cost_usd.toFixed(6)}`);
    });
    
    // Batch estimate
    const batchEstimate = estimateEmbeddingCost(sampleTexts);
    console.log(`   - Batch Total: ${batchEstimate.tokens} tokens, $${batchEstimate.cost_usd.toFixed(6)}`);
  } catch (error) {
