...
console.log('Embedding model:', process.env.OPENAI_EMBEDDING_MODEL || 'text-embedding-3-small (default)');
...
try {
  ...
  console.log(`   - Embedding sample: [${embedding.embedding.slice(0, 3).map(n => n.toFixed(4)).join(', ')}...]`);
  ...
  const chatResponse = await openai.chat.completions.create({
    ...
    messages: [{
      role: 'user',
      content: 'Analyze this text for quality (1-10): "' + testText + '". Respond with JSON: {"score": 8, "reason": "clear and concise"}'
    }],
  ...
  const chatCost = (chatUsage?.prompt_tokens || 0) * inputCostPerToken + 
  ...
  scenarios.forEach(scenario => {
    ...
    console.log(`   - ${scenario.name}: $${totalCost.toFixed(4)} (embeddings: $${embeddingCost.toFixed(4)}, AI analysis: $${aiCost.toFixed(4)})`);
  });
