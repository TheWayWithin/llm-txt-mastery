...
async function testUserFlows() {
  ...
  try {
    ...
    await page.waitForSelector('text=/Analyzing|Processing|Discovering|Fetching/i', {
      timeout: 15000,
    });
    ...
    const analysisComplete = await page
      .waitForSelector('text=/Review|Complete|Select|found/i', {
    ...
    const usageText = await page
      .locator('text=/1.*of.*1|1\/1|Daily limit/')
      .textContent()
      .catch(() => null);
    if (usageText) {
    ...
    const tierStatus = await page
      .locator('text=/Coffee|Unlimited daily analyses/i')
      .first()
      .textContent()
      .catch(() => null);
    if (tierStatus) {
