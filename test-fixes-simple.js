...
async function testLLMTxtMastery() {
  ...
  try {
    ...
    const quickStartButton = page
      .getByRole('button', { name: /quick start|continue without|no password/i })
      .first();
    ...
    if (!results.dailyLimitEnforced) {
      ...
      const analysisStarted = await page
        .locator('text=/analyzing|discovering|processing/i')
...
async function performAnalysis(page, url) {
  try {
    // Enter URL
    const urlInput = page
      .locator('input[placeholder*="website"], input[placeholder*="URL"], input[type="url"]')
      .first();
